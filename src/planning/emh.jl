...
for t in T
    ...
    CSV.write("GridOpt.jl/results/emh/new_gen_dispatch_t$(years[t]).csv", DataFrame(results[:pC][:,:,t].data, :auto))
    CSV.write("GridOpt.jl/results/emh/exist_gen_dispatch_t$(years[t]).csv", DataFrame(results[:pE][:,:,t].data, :auto))
end
...
savefig(po, "GridOpt.jl/results/emh/cumulative_capacity.pdf")
...
savefig(po2, "GridOpt.jl/results/emh/total_emissions.pdf")